...
def main():
    data_dir = os.path.join(THIS_DIR, 'data', 'alegre') #os.path.join(THIS_DIR, 'data', 'ted500')
    class_names = [c.split('.')[-1] for c in glob.glob('data/alegre/mr.*')] #[c.split('.')[-1] for c in glob.glob('data/ted500/ted_500.*')]
    reader = TextReader(data_dir, class_names=class_names)
    #reader.prepare_data(vocab_size=4090, test_size=50)
    reader.prepare_data(vocab_size=150, test_size=20)
