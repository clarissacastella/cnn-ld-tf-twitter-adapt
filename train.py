...
import cnn
import util
#Split dataset into train/test set: 672 for training, 120 for evaluation.
#{'train_size': 672, 'vocab_size': 150, 'max_sent_len': 1763, 'class_names': ['fr', 'en', 'pt', 'it', 'ar', 'es'], 'test_size': 120}
#Loaded target classes (length 6).
#Loaded data with 672 examples. 100 examples per batch will be used.
#Preparing test data ...
#Loaded target classes (length 6).
#Loaded data with 120 examples. 100 examples per batch will be used.
#Parameters:
 #         vocab_size 4090
  #        max_window 5
   #         data_dir '/media/ccx/faa8fb7c-c759-4e71-8525-d0d7a7a6dbba3/work/lang_detect/cnn-ld-tf_twitter/data/alegre'
    #    summary_step 200
     #     min_window 3
      #    num_kernel 100
       #    train_dir '/media/ccx/faa8fb7c-c759-4e71-8525-d0d7a7a6dbba3/work/lang_detect/cnn-ld-tf_twitter/model/alegre'
       #   batch_size 100
       #     log_step 10
       #     emb_size 300
       #    num_epoch 20
...
tf.app.flags.DEFINE_string('data_dir', os.path.join(this_dir, 'data', 'alegre'), 'Directory of the data')
tf.app.flags.DEFINE_string('train_dir', os.path.join(this_dir, 'model', 'alegre'), 'Directory to save training checkpoint files')
tf.app.flags.DEFINE_integer('num_epoch', 20, 'Number of epochs to run')
...
def train():
    ...
    config['num_classes'] = 6 #train_loader.num_classes
    ...
    with tf.Graph().as_default():
        ...
        if FLAGS.use_pretrain:
            ...
            pretrained_embedding = util.load_embedding(emb_file, vocab_file, 150)
